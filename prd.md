...
**Relocated 2026-09-18: the whole cluster follows EL.** Four independent lines now point the same way, one in each telemetry wave, and none points to the DXF. (1) The WB01-INV07 rail stencils above (wave 4). (2) The cleaning log: crews record strings by their physical labels, and on 12 September they cleaned 19 strings of WB01-INV20/21 (sheet `STS1 (2026 Cycle 3)`); the drone frame taken at 16:33 that day shows exactly 19 freshly cleaned blue tables among dusty beige ones. At the EL positions the log agrees 14/14 on the points that land on a table; at the DXF positions 13/29, which is chance (INV20 wave 3, INV21 wave 2). (3) Eleven DXF tables of the cluster receive no EL point at all, and the seven the camera saw (0216/0217) are grass or bare ground. (4) Eleven EL points fall outside every DXF table, and the six in view land on physical tables that the DXF does not draw. The EL points of the 13 inverters fall only on tables the DXF assigns to those same 13, so the move is a closed permutation and no other WB01 inverter is touched.

`PLACEMENT_FROM_EL` in `build_string_geometry.py` now carries the 13 WB01 inverters alongside the four WB02 ones. A relocated string takes the centre of the DXF table that contains its EL point, unless that table belongs to a string that did not move or a second relocated string lands in it; otherwise it stays at its EL point without a table centre (11 in WB01, 12 in WB02). Measured after regeneration: the 234 WB01 positions move by a median 30.3 m (maximum 82.5 m); collisions under 3 m across Phase One stay at 7 before and after; the ground under the cluster stays flat (|cross-slope| median 0.55, maximum 2.05 degrees); and with the new geometry the 0216 frame matches the cleaning log on all 66 strings of WB01-INV18..21. Seasonal verdicts barely move (one string, WB01-INV08), because Phase One is flat — the value of the relocation is where the strings physically are, which is what visit lists, drone identity and soiling maps use. Any WB01 coordinate taken from the geometry before this date is stale.

**WB02 completed 2026-09-19: the Open Question 8 cluster had seven members, not four.** WB02-INV03, INV07 and INV08 disagree with EL by a median 16.2, 16.2 and 14.6 m, and the EL points of all seven inverters fall almost entirely on tables the DXF assigns to the same seven (114 of 126; 11 outside every table, one on a WB02-INV05 table). Moving only the four, as OQ8 did, is the partial relocation this section warns about: 12 of the 72 OQ8 strings could not take a table because WB02-INV03/07/08 still held it at their DXF positions. Photographic evidence comes from the drone archive on the external disk (`F:\Drone DJI Mini 5 Pro`, 9-15 September; the northern edge is only seen obliquely or near-nadir). Across frames that contain both, the 12 DXF tables that receive no EL point show a median module fraction of 0.19 against 0.59 for cluster tables that do; and in the near-nadir frame `DJI_20260909171650_0100` the six EL points in view that lie outside every DXF table sit on physical tables the DXF does not draw — the same signature as WB01. Single-point colour tests are not sharp in oblique frames (inter-row gaps are hidden by the next row: gap control 0.60 against table control 0.63), so the area test is the one relied on. After adding the three inverters to `PLACEMENT_FROM_EL`: their 54 strings move by a median 15.6 m onto flat ground (|cross-slope| median 0.55, maximum 1.58 degrees); the cluster has 114 of 126 strings on a table (was 60 of 72 for OQ8 alone); and collisions under 3 m across Phase One fall from 7 to **1** — the other six were created by the partial move. No seasonal or June verdict changes. WB02-INV05 agrees with EL (median <= 2 m) and is not moved.

**Open, 2026-09-19: WB08-INV06/07/08 — the two derived sources disagree, and no physical source is available yet.** Unlike WB01 and WB02, every WB08 EL point falls inside a 1129.dxf table and no table is left without one; what differs is the label assignment. EL puts WB08-INV07-STn on the table the DXF labels ST(n+1), for all 25 strings, with the same one-table shift on the tail of INV06 (ST18-25) and the head of INV08 (ST01-03). The DXF side is internally strained: the first label of each INV07 row (ST01/05/11/17/23, plus INV06-ST18) floats 5.6-9.1 m west of every table, and the easternmost table of each row carries no label. The as-built DC cable lengths point the other way, decisively: with the inverter placed at its drawn block (not its text, which sits ~12 m west of the block throughout this group), cable length against Manhattan distance gives R² 0.95 at the DXF-label positions and 0.39 at the EL positions for INV07 (INV06 0.92/0.77, INV08 0.95/0.90; the 14 undisputed WB08 inverters 0.53-0.99, median ~0.94). Neither source is clearly a field measurement — the cable lengths may be design lengths computed from the label layer, and the EL coordinates may derive from the table layer — so this is two layers of one drawing disagreeing, not a verdict. The only drone frame that sees these tables (`DJI_20260914111255_0028`, oblique -49.5° from 118 m) is too weak to decide (one hypothetical table read 0.30 against 0.68 for controls). **The geometry is left as it is**, which already follows the DXF labels. One label read settles it: at the westernmost table of the second INV07 row (-0.986481, 116.637692) EL predicts `WB08-INV07-ST05` and the DXF `WB08-INV07-ST06`; a nadir photo at about 40 m over the east row end (-0.986481, 116.638386) shows whether that table exists at all. The points are in `coba/titik_verifikasi_wb08.csv`.

**Decided, 2026-09-20: WB03-INV08 ST16-21/23-25 and WB03-INV09-ST02 follow the EL survey; the same cable test that defended the DXF in WB08 overturns it here.** The 1129.dxf label layer leaves `WB03INV08ST16` and `WB03INV09ST02` floating outside every table rectangle and one rectangle in that run unlabelled; the EL survey puts INV08-STn on the rectangle the DXF labels ST(n+1) across ST16-21 and ST23-25, fills the unlabelled rectangle with ST21, and puts INV09-ST02 on the rectangle labelled INV08-ST23 — every string gets exactly one table. The as-built cable lengths agree with the EL survey, not with the labels: calibrating length against Manhattan distance to the drawn inverter block on the 16 strings where both sources agree gives R² 1.00 with a 0.7 m residual sd, and the disputed strings then miss by +7.9 to +20.7 m (about one table length) at the DXF label positions against +0.2 to +2.3 m at the EL positions (INV09-ST02: +7.9 vs -0.7 m, sd 2.3 m; the other 13 WB03 inverters score R² 0.58-1.00, median ~0.98). Two derived sources against one layer of one drawing is not a field reading, but it is the reverse of WB08, where the cable lengths backed the labels — so the scope is **per string** (`PLACEMENT_FROM_EL_STRING`), not a rule that the survey always wins. Regenerated geometry: 10 strings moved, 6 more labels gained a table centre (3563/3570), no two strings share a rectangle, and `WB03-INV08-ST25` keeps its EL point without a table centre because no unclaimed rectangle contains it. Separately, five labels that sit ~0.4 m west of their own unlabelled table (WB05-INV17 ST09/14/19/24, WB10-INV15 ST22) now take that table's centre when the string's EL point falls inside it (`attach_table_centers_via_el`); WB08's six floating labels are untouched because their EL points land on tables that already carry another string's label.


**Extended 2026-09-20: the chain ends at `WB03-INV11-ST04`, which is why ST25 had no table.** The rectangle containing ST25's EL point is drawn — its centre is 1.3 m away — but the DXF labels it `WB03INV11ST04`, so the rectangle was claimed, not absent. The EL survey puts INV11-ST04 one rectangle further east, on the unlabelled one at the end of that row, and the offset is not ambiguous: the other 24 INV11 strings sit -1.05 ± 0.81 m east and -1.13 ± 0.04 m north of their labelled rectangle's centre, where ST04 sits +14.64 m east — about one table pitch (15.45 m) — with the same northing offset (-1.17). The cable test repeats the WB03 verdict rather than assuming it: calibrated on those 24 agreeing strings (Manhattan distance to the fitted inverter block, R² 0.998, residual rms 0.75 m), the recorded 29 m for ST04 falls 17.8 m short of the 46.8 m predicted at the DXF label position (23.5 sigma) and 0.9 m over the 28.1 m predicted at the EL position (1.2 sigma); a Euclidean routing model agrees on the direction but fits far worse (R² 0.948), so the cable follows the rows. Adding `("WB03-INV11", 4)` to `PLACEMENT_FROM_EL_STRING` moves ST04 onto the unlabelled rectangle and hands its old one to INV08-ST25: 11 strings now move, labels with a table centre rise to 3564/3570, and no WB03 string is left without one — site-wide the count falls 30 -> 29, all of them now in Phase One and WB08 (WB01 11, WB02 12, WB08 6). Two side effects follow from the ground plane being fitted under the table centre rather than the label, and neither is suppressed: ST25's plane now comes from the real rectangle instead of the fraction-formula footprint, moving its cross-slope from -9.64 to -6.31 degrees, and ST04's plane at its new rectangle misses the 0.5 m rms gate (0.555 m), so its slope columns are emitted as NULL and site-wide clean-plane coverage falls 4411 -> 4410. Any `WB03-INV11-ST04` coordinate taken from the geometry before 2026-09-20 is stale.
