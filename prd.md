...
- Absent telemetry is not downtime. A day on which Phase One (WB01/WB02) is
  ...
  **A third failure class, found 2026-08-15 and not yet discriminated.** The
  ...
  transfer artefact dressed as equipment failure. Until a discriminator exists,
  confirm any `INVERTER_ABSENCE` against a second copy of the same date before
  acting on it — compare the total row count first, since a whole-inverter
  shortfall is an exact multiple of the per-inverter row count.
...
The propagation is **7 of 72 strings, not the set**: six of WB02-INV02 land about 2.0 m from WB02-INV03 rows and one of WB02-INV04 lands 1.9 m from a WB02-INV05 row. That is a real collision rather than ordinary spacing — Phase One's nearest-neighbour distance is a uniform 7.27 m (p5 = p95) and no string anywhere on the plant normally has a neighbour within 3 m. It was applied regardless, because the collision is a **labelling** question and the plane columns answer a **terrain** question: those seven differ from the neighbours they land on by only 0.75-1.93 degrees of cross-slope, on ground that is flat everywhere in Phase One, so which of the two labels owns the position does not change the terrain value written. What stays open is the label ownership for those seven, and it now has evidence on both sides.
